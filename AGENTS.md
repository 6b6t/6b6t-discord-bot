...
Rust Discord bot for the 6b6t community. The bot serves the Discord server around the Minecraft servers and the website.
...
- Format: `cargo fmt`
- Run locally: needs Discord credentials from the operator; do not commit tokens.

## Git and pull requests

- Conventional Commits: `type(scope): description`. CI validates commit subjects on every PR.
- One logical change per PR, based on the default branch (`main`). See the boundaries below.
- Reviews are routed via `.github/CODEOWNERS`; open PRs against `main`.
