...
export async function getLatestVideo(queries: string[], ignoreWords: string[]): Promise<{ author: string, title: string; url: string } | null> {
    try {
        const query = queries.join(' OR ') + ' ' + ignoreWords.map(word => `-` + word).join(' ');
        ...
        if (response.data.items && response.data.items.length > 0) {
            ...
            const hasQuery = queries.some(query =>
                title.toLowerCase().includes(query.toLowerCase()) ||
                description.toLowerCase().includes(query.toLowerCase())
            );

            if (videoId && hasQuery) {
                return { author, title, url: `https://www.youtube.com/watch?v=${videoId}` };
            }
        }
    } catch (error) {
...
export async function sendYoutubeNotification(channel: TextChannel, queries: string[], ignoreWords: string[]) {
